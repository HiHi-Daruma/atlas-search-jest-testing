...
describe("searchMovies", () => {
  ...
  let db: Db;
  let movieCollection: Collection<Movie>;
  ...
  beforeAll(async () => {
    ...
    db = client.db("test");
    movieCollection = db.collection<Movie>("movies");

    await insertManyInAtlasSearch(movieCollection, testMovies, searchIndexSpec);
  });
  ...
  it("should find movies by title", async () => {
    const results = await searchMovies(movieCollection, "Inception");
    expect(results).toEqual([testMovies[0]]);
  });

  it("should find movies by genre", async () => {
    const results = await searchMovies(movieCollection, "Sci-Fi");
    expect(results).toEqual(
      expect.arrayContaining([testMovies[0], testMovies[2], testMovies[3]]),
    );
  });

  it("should find movies by plot", async () => {
    const results = await searchMovies(movieCollection, "Joker");
    expect(results).toEqual([testMovies[1]]);
  });

  it("should return empty array when no results found", async () => {
    const results = await searchMovies(movieCollection, "NonExistentMovie");
    expect(results).toEqual([]);
  });
});
